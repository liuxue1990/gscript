            ...
            for (int i = 0; i < sampleFeaturesMap.length; ++i) {
                for (int j = i + 1; j < sampleFeaturesMap.length; ++j) {
                    abMap[partIndex][i][j] = computeAB(
                            parts[partIndex].getTemplate().getFeatures(),
                            GSMath.normalize(sampleFeaturesMap[i][j].getFeatures(), null));
                }
            }
        }
...
//                    System.out.println((a * Math.cos(angle) + b * Math.sin(angle)) + " vs " + distanceToTemplateAtAngle(template, features, angle));
